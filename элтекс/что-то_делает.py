...
def Loading():
    layout = [ [sg.Text('Loading....', font='ANY 15')],
     [sg.Image(data=gif103, key='_IMAGE_', enable_events=True, background_color='white')],
     [sg.Button('Cancel')]
     ]
...
thr1 = threading.Thread(target = statistics_for_the_week, daemon=True)
thr2 = threading.Thread(target = Loading, daemon=False)

thr1.start()
thr2.start()
#thr1.join(60.0)

while shared_resource == True: 
    thr2.terminate()
...
"""while True:
    if thr1.is_alive() == True: 
        thr2.start()
        #print(";ltv")# Проверяем, выполняется ли поток демон
        #time.sleep(1) # Если да, ждем 1 секунду и проверяем снова
        
    if thr1.is_alive() == False:
        thr2.terminate()
        break #"""
